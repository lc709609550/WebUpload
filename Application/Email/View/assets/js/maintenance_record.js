...
$(function () {
    ...
    //查询
    maintenance_record_query();

});

//表格创建
function maintenance_record_table() {
    //绘制表格
    ...
    $("#jqGridList").jqGrid({
        ...
        ondblClickRow:function(){
            $('.modalWrap').html('');
            $.ajax({
                'url': './maintenance_record_details.html',
                "type": "POST",
                'success': function (data) {
                    $('.modalWrap').html(data);
                    $('#maintenance_record_details').modal('show');
                }
            });
        }
    });
...
//查询
function maintenance_record_query(){
    $('.maintaining_record_query').unbind().bind('click',function(){
        $('.modalWrap').html('');
        $.ajax({
            'url': './maintenance_record_query.html',
            "type": "POST",
            'success': function (data) {
                $('.modalWrap').html(data);
                $('#maintenance_record_query').modal('show');
            }
        });
    });
}
